#define VERSION "\nDomeBuddy Controller MVC V1.221206 by keithrickard@hotmail.com"
...
//  enc360 = 18432;                                 // 1024 * 1350 / 75
  ...
  scopeAZM = domeAZM = homeAZM = lastTargAZM = enc2azm(currEnc);
...
void loop() {
  ...
  if (!parked) moveDome(scopeAZM, 0);               // If not parked, dome if necessary. (0 = no rollback, landing tolerance)
...
//==============================================================================================================
void moveDome(byte targAZM, byte rb) {                          // rb = 1 means rollback is required
  if (domeAZM == targAZM || lastTargAZM == targAZM)  return;    // Return if target is same dome or last target azimuth
  ...
  if (!rb && dir != lastDir && abs(distEnc) <= ROLLBACK * encOne) {
    ...
    if (byte(domeAZM - targAZM) > 1 && byte(targAZM - domeAZM) > 1) { //**
      Serial.println("\nROLLING BACK...");
      currEnc = prevEnc;
...
//  Serial.println(enc360);
  ...
  domeAZM = lastTargAZM = enc2azm(normEnc());     // Normalise currEnc and update domeAZM
  Serial.print(domeAZM);
  Serial.print("=");
  Serial.println(currEnc);
  ...
  if (byte(domeAZM - targAZM) <= 1 || byte(targAZM - domeAZM) <= 1) lastTargAZM = targAZM;  // Tolerance test.
}
...
void getScopeAZM() {                                              // Get and interpret scope's azimuth from DomeBuddy Dongle via Bluetooth
  ...
  if (value == byte(~chksum)) {
    ...
    Serial.print("=");
    Serial.println(azm2enc(scopeAZM));
  }
